...
APP_NAME=${APP_NAME:-SafeRun}
BUNDLE_ID=${BUNDLE_ID:-com.jelwin.safe-run}
...
build_product_path() {
  local name="$1"
  local arch="$2"
  local capitalized_conf="$CONF"
  case "$CONF" in
    release) capitalized_conf="Release" ;;
    debug) capitalized_conf="Debug" ;;
  esac
  local candidates=(
    ".build/${arch}-apple-macosx/$CONF/$name"
    ".build/$CONF/$name"
    ".build/out/Products/$capitalized_conf/$name"
  )
